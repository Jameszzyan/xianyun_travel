<template>
  <div class="aside">
    ...
    <div class="history">
      <h5>历史查询</h5>
      <nuxt-link :to="`/planeTicket/flights?departCity=${item.departCity}&departCode=${item.departCode}&destCity=${item.destCity}&destCode=${item.destCode}&departDate=${item.departDate}`" v-for="(item,index) in history" :key="index">
...
export default {
  ...
  mounted(){
    this.history = this.$store.getters['history/getHistory']
